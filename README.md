...
## Окна
Конечным родителем для всех кнопок является класс [`Window`](https://github.com/quqveik1/TESTWIN32_GRAPHICAPP/blob/main/Window.h). Какие события вы можеет переопредлеить, не забудьте про вызов родительской функции.
```cpp
...
```
Особенностью всех таких окон - наличие собственного холста и своего изображения [`M_HDC`](https://github.com/quqveik1/TESTWIN32_GRAPHICAPP/blob/main/M_HDC.h). Они рисуют внутри него в функции `draw()`. А потом переносят в функции `print(M_HDC& finalDC)` на родительский холст. Это не всегда достаточно быстро, поэтому есть класс [`Layout`](https://github.com/quqveik1/TESTWIN32_GRAPHICAPP/blob/main/Layout.h), который не имеет собственного холста и рисует сразу в функции `print(M_HDC& finalDC)`. Также присутсвют такие вещи как [`LinearLayout`](https://github.com/quqveik1/TESTWIN32_GRAPHICAPP/blob/main/LinearLayout.h), для удобной компоновки интерфейса.
...
6. Программе необоходим минимум С++17, рекомендуется сделать так. Properties -> C++ -> Command Line -> Введите `/std:c++latest`
...
7. Properties -> Advanced -> Character Set -> Поменяйте Use Unicode Character Set -> Use Multi-Byte Character Set
 ...
 8. Создайте класс наследник [`AbstactAppData`](https://github.com/quqveik1/TESTWIN32_GRAPHICAPP/blob/main/AbstractApp.h).
    ...
    void TestEngine::onCreate(HWND window, UINT message, WPARAM wParam, LPARAM lParam)
    {
